...
client = TelegramClient(f'{tmp_path}/.{account["username"]}_tg_login', account['api_id'], account['api_hash'],
                        proxy=proxy)
client.start(phone=account['phone'])
# client.start()

# 设置bot，且直接启动
bot = TelegramClient(f'{tmp_path}/.{account["bot_name"]}', account['api_id'], account['api_hash'], proxy=proxy).start(
...
async def on_greeting(event):
    ...
    if True:  # 所有消息类型，支持群组
        ...
        logger.debug(
            f'event.chat.username: {get_event_chat_username(event_chat)},event.chat.id:{event_chat.id},{_title} event.message.id:{event.message.id},text:{text}')
        ...
        sql = f"""
      select u.chat_id,l.keywords,l.id,l.chat_id,l.target_channel
        ...
        if find:
            ...
            for receiver, keywords, l_id, l_chat_id, target_channel in find:
                try:
                    ...
                    # 优先返回可预览url
                    channel_url = f'https://t.me/{event_chat_username}/' if event_chat_username else get_channel_url(
                        event_chat_username, event.chat_id)
                    ...
                    if isinstance(event, events.MessageEdited.Event):  # 编辑事件
                        ...
                        if cache.get(send_cache_key) and (
                                event.message.edit_date - event.message.date) > datetime.timedelta(seconds=2):
                    ...
                    if is_regex_str(keywords):  # 输入为正则字符串
                        ...
                        elif regex_match is not None:  # 如果使用findall()并返回列表
                            regex_match = list(regex_match)
                        else:
                            regex_match = []  # 如果没有匹配结果，设为空列表
                    ...
                    if user_id:
                        isdel2 = utils.db.user_subscribe_list.delete().where(
                            utils.User_subscribe_list.user_id == user_id.id).execute()
            ...
            if find_channel:
                for i in find_channel.groups():
                    if i:
                        channel = i
                        break
            res.append((keyword, channel))  # 只返回关键字和频道
    return res
    ...
    for k, c in keyword_channel_list:  # 确保只解包关键字和频道
        ...
        try:
            if c.lstrip('-').isdigit():
                ...
                chat_id = telethon_utils.get_peer_id(PeerChannel(real_id))
            else:
                channel_entity = await client_get_entity(c, time.time() // 86400)
                chat_id = telethon_utils.get_peer_id(PeerChannel(channel_entity.id))
...
async def subscribe(event):
    ...
    if len(splitd) <= 1:
        await event.respond(
            r'输入需要订阅的关键字,支持js正则语法：`/[\s\S]*/ig`\n\nInput the keyword that needs to subscribe, support JS regular syntax：`/[\s\S]*/ig`')
        cache.set('status_{}'.format(chat_id), {'current_status': '/subscribe keywords', 'record_value': text},
                  expire=5 * 60)
    elif len(splitd) == 3:
...
async def unsubscribe(event):
    ...
    if len(splitd) <= 1:
        ...
        cache.set('status_{}'.format(chat_id), {'current_status': '/unsubscribe keywords', 'record_value': text},
                  expire=5 * 60)  # 设置5m后过期
    elif len(splitd) == 3:
        command, keywords, channels = splitd
...
async def setlengthlimit(event):
    ...
    find = utils.db.connect.execute_sql(
        ...
        (user_id.id, blacklist_type)).fetchone()
    if not splitd:
        if find is None:
        ...
        if len(splitd) == 1 and splitd[0].isdigit():
            ...
            if find is None:
                ...
                update_query = utils.db.user_block_list.update(blacklist_value=blacklist_value,
                                                               update_time=datetime.datetime.now().strftime(
                                                                   '%Y-%m-%d %H:%M:%S')).where(
                  utils.User_block_list.id == find[0])  # 更新状态
    ...
    if find:
        ...
        if find['current_status'] == '/subscribe keywords':  # 当前输入关键字
            ...
            cache.set('status_{}'.format(chat_id),
                      {'current_status': '/subscribe channels', 'record_value': find['record_value'] + ' ' + text},
                      expire=5 * 60)  # 记录输入的关键字
            raise events.StopPropagation
        ...
        # 取消订阅
            ...
            cache.set('status_{}'.format(chat_id),
                      {'current_status': '/unsubscribe channels', 'record_value': find['record_value'] + ' ' + text},
                      expire=5 * 60)  # 记录输入的关键字
            raise events.StopPropagation
